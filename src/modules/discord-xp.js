...
module.exports = class DiscordXp {
  constructor() {
    ...
    const computedArray = leaderboard.map((key) => {
      return {
        ...
        discriminator: client.users.cache.get(key.userID)
          ...
          : "0000",
        mention: client.users.cache.get(key.userID)
          ? client.users.cache.get(key.userID).toString()
          : `<@!` + key.userID + `>`,
        tag: client.users.cache.get(key.userID)
          ? client.users.cache.get(key.userID).tag
          : "Unknown (" + key.userID + ")",
      };
    });
