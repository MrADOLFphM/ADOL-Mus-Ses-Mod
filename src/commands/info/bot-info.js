...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const avatar = await loadImage(
      client.user.displayAvatarURL({ format: "png", size: 64 })
    );
    ...
    ctx.fillStyle = "rgba(48, 4, 110, 0.9)";
    ctx.fillText(commatize(client.messages.received), 200, 125, 100);
    ctx.fillText(commatize(client.messages.sent), 200, 155, 100);
    ctx.fillText(commatize(client.guilds.cache.size), 200, 185, 100);
    ctx.fillText(commatize(client.commands.size), 200, 215, 100);
    ...
    ctx.font = "10px arial";
    ctx.fillText(`[ ${commatize(messages_cached)} messages ]`, 75, 175, 120);
    ...
    ctx.arc(
      ...
      50,
      Math.PI * 1.5,
      Math.PI * 1.5 + Math.PI * 2 * (heapTotal / 1024 / 1024 / MemoryLimit)
    );
    ctx.strokeStyle = "rgba(48, 4, 110, 0.8)";
    ctx.stroke();
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      Math.round(100 - (heapTotal / Math.pow(1024, 2) / MemoryLimit) * 100),
      320,
      150,
      40
    );
    ...
    ctx.font = "10px arial";
    ctx.fillText(
      `[ ${(heapTotal / Math.pow(1024, 2)).toFixed(2)} MB used ]`,
      320,
      175,
      80
    );
    ...
    return message.channel.send("Bot Stats for Nerds", {
      embed: {
        color: 3146862,
        description: `[Github](https://github.com/tovade/Andoi)\u2000\u2000`,
      },
