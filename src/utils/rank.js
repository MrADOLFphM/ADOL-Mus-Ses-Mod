...
module.exports = async (
  { username, discrim, level, rank, neededXP, currentXP, avatarURL },
  color = "FFFFFF"
) => {
  if (!username) throw new Error("No username was provided!");
  if (typeof level !== "number" && !level)
    throw new Error("No level was provided!");
  ...
  Canvas.registerFont(__dirname + "/bold-font.ttf", {
    family: "Manrope",
    weight: "regular",
    style: "normal",
  });
  Canvas.registerFont(__dirname + "/regular-font.ttf", {
    family: "Manrope",
    weight: "bold",
    style: "normal",
  });
  ...
  ctx.textAlign = "start";
  const name =
    username.length > 9 ? username.substring(0, 8).trim() + "..." : username;
  ...
  ctx.fillText(
    "/ " + neededXP,
    624 + ctx.measureText(currentXP).width + 10,
    164
  );
  ctx.fillStyle = "#" + color;
